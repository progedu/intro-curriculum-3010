...
/**
 * タスクをファイルに保存する:
 * tasks という連想配列を Array.from で配列に変換した後、 
 * JSON.stringify という関数で JSON の文字列に変換、
 * さらに同期的にファイルに書き出しをしている。
 */
function saveTasks() {
...
*/
function list() {
  return Array.from(tasks) //tasksを配列に変換
    .filter(isNotDone) //falseのみになるようフィルタにかける    
    .map(t => t[0]); 　//マップでタスクのみ返す
}
